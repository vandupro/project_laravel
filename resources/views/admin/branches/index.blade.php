    ...
    <div class="panel panel-default">
        ...
        <div style="display: flex; justify-content:space-between; margin: 20px 20px; border: 1px solid #cccc; padding: 10px;">
            ...
            <form style="display:flex;" action="" method="GET">
                ...
                <div class="ml-3">
                    <a class="btn btn-info" href="{{route('admin.branches.index')}}">Hủy</a>
                </div>
            </form>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-striped b-t b-light">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{(($branches->currentPage()-1)*$pagesize) + $loop->iteration}}</td>
                        <td>{{$c->name}}</td>
                        <td>{{count($c->products)}}</td>
                        <td><span class="text-ellipsis">{{$c->desc}}</span></td>
                        <td><span class="text-ellipsis">
                                @if($c->status)
                                <a href="{{route('admin.branches.active', ['id'=>$c->id])}}"><span
                        ...
                        <td style="width: 15%">
                            @hasanyrole('Content|Admin')
                            <a class="btn btn-warning" href="{{route('admin.branches.edit', ['id'=>$c->id])}}">Sửa</a>
                            <a onclick="return confirm('Bạn có chắc muốn xóa không?')" 
                            class="btn btn-danger" href="{{route('admin.branches.delete', ['id'=>$c->id])}}">Xóa</a>
                            @else
                            Not authorized
                            @endhasanyrole
                        </td>
